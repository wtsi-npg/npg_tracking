...
sub _build_is_pool {
  my $self = shift;
  # Tag zero and a lane pools.
  return $self->position ? ($self->tag_index ? 0 : 1) : 0;
}
...
sub _build__product_row {
  ...
  # row is returned. For consistency the retrieved rows are ordered alphabethically
  # by tag sequence. In future we consider assigning children to entities like this.
  ...
  if ($self->tag_index) {
    ...
    $rs->count or croak 'No database record retrieved for ' . $self->to_string();
