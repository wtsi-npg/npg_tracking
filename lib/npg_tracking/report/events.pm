  ...
  while ( my $event = $unprocessed_events->next() ) {
    ...
    if ( ($rs_name =~ /Result::Run/xms) && $entity->can('id_run') ) {
      ...
      if ($manufacturer ne 'Illumina') {
        ...
        if ($entity->can('is_reportable') && $entity->is_reportable()) {
          $report_type = 'followers';
        }
      }
    } else {
...
sub _get_report_obj {
  ...
  if ($self->schema_mlwh() && ($report_type eq 'followers')) {
...
=head1 DESCRIPTION
 ...
 In 2025 events were generated by run status changes, instrument status
 changes, run, run-lane and instrument annotations.

 Instrument-related emails are sent to users who are memebers of the
 tracking database group 'engineers'.

 Run-related emails are sent to users associated with the studies the
 samples on the run belong to.
...
Copyright (C) 2017,2021,2023,2025,2026 Genome Research Ltd.
