...

{
    ...
    my $r = npg_tracking::daemon::staging->new(root_dir => $root_dir, timestamp => '2013', hosts => [$hostname, 'sf3-nfs']);
    ...
    is($r->host_name2path($hostname), qq[$root_dir/sf2], 'get sf area from host');
    is($r->command($hostname), qq[$script $root_dir/sf2], 'command to run');
    throws_ok {$r->host_name2path('seq3')}
...

{
    my $hostname = q[sf18-nfs];
    my $root_dir = q[/nfs];
    my $log_dir = qq[$root_dir/sf18/staging_area_monitor_logs];
