...

{
  ...
  cmp_ok($ss->output, 'eq', '/nfs/sf45/ILorHSorMS_sf45/samplesheets/MS0001309-300.csv', 'default output location (with zeroes trimmed appropriately)');
}
{
  my $ss;
  lives_ok { $ss = npg::samplesheet->new(npg_tracking_schema=>$schema, id_run=>7007); } 'sample sheet object - no output provided';
  ...
  cmp_ok($ss->output, 'eq', '/nfs/sf45/ILorHSorMS_sf45/samplesheets/MS2000132-500V2.csv', 'default output location copes with V2 MiSeq cartirdges/reagent kits');
...
}
{
  ...
  cmp_ok($ss->output, 'eq', '/nfs/sf45/ILorHSorMS_sf45/samplesheets/000000000-A0616.csv', 'default output location')
}
{
  my $ss;
  my $result = q();
