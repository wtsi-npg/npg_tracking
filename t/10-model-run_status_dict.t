...

{
  ...
  my $current_run_status = q(analysis pending);
  my $id_run = 11;

  my $rsds = $rsd->run_status_dicts_sorted_feasible(11);
  my $rsds_length = scalar @{$rsds};

  isa_ok($rsds, 'ARRAY');
  is(($rsds_length), 14, 'run status that are possbile from this status');
