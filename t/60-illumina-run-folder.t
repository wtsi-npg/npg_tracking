...
sub create_plex_staging {
  my ($qc_subpath, $basecalls_subpath, $config_path, $archive_subpath, $lanes, $qc_lanes) = @_;
  create_staging($qc_subpath, $basecalls_subpath, $config_path);
  foreach my $lane (@{$lanes}) {
    my $dir = qq[$archive_subpath/lane$lane];
    `mkdir $dir`;  
  }
  foreach my $lane (@{$qc_lanes}) {
...
sub create_latest_summary_link {
  my ($qc_subpath, $basecalls_subpath, $config_path, $gerald_subpath, $runfolder_path) = @_;
  create_staging($qc_subpath, $basecalls_subpath, $config_path);
  `ln -s $gerald_subpath $runfolder_path/Latest_Summary`;
  return 1;
}

sub create_staging_no_recalibrated {
...
sub create_staging_PB_cal {
  my ($bustard_subpath, $basecalls_subpath, $config_path) = @_;
  create_staging_no_recalibrated($bustard_subpath, $basecalls_subpath, $config_path);
  `mkdir $bustard_subpath/PB_cal`;
  return 1;
}

# loops over all remaining tests for both IL and HS
foreach my $instr (qw/IL2 HS2/) {

  note q{Testing loop for } . $instr;

  my $id_run = q{1234};
  my $name = $instr . q{_1234};
  my $run_folder = q{123456_} . $instr . q{_1234};
  if ( $instr eq q{HS2} ) {
    $run_folder .= q{_B_205NNABXX};
  }
...

{
  ...
  is($path_info->runfolder_path(), $runfolder_path, q{runfolder_path found});
  is($path_info->recalibrated_path(), $gerald_subpath, q{recalibrated_subpath found when subpath is/is below recalibrated directory});
}

{
  create_latest_summary_link($qc_subpath, $basecalls_subpath, $config_path, $gerald_subpath, $runfolder_path);
...

{
  my $lanes = [1,3,5,7];
  my $qlanes = [1,5,7];
  create_plex_staging($qc_subpath, $basecalls_subpath, $config_path, $archive_subpath, $lanes, $qlanes);
...
{
  create_staging_PB_cal($bustard_subpath, $basecalls_subpath, $config_path);
  ...
  `mkdir -p $bustard_subpath/GERALD_20100617`;
  $path_info = test::run::folder->new({ id_run => $id_run, run_folder => $run_folder, });

  throws_ok { $path_info->recalibrated_path(); } qr{found[ ]multiple[ ]possible[ ]recalibrated_directories}, q{more than one path found};
  ...
  is( $path_info->bustard_path(), qq{$bustard_subpath}, q{Help directory supplied as recalibrated_path, bustard worked out ok, so this is to be used} );
  is( $path_info->recalibrated_path(), qq{$bustard_subpath/Help}, q{Help directory supplied as recalibrated_path, so this is to be used} );
...
qx{mkdir -p $hs_runfolder_dir/Data/Intensities/PB_basecalls_20101016-172254/no_cal/archive};
qx{mkdir -p $hs_runfolder_dir/Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2/GERALD_01-10-2010_RTA/archive};
qx{mkdir -p $hs_runfolder_dir/Config};
qx{ln -s Data/Intensities/PB_basecalls_20101016-172254/no_cal $hs_runfolder_dir/Latest_Summary};

{
  note( $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254/no_cal/archive));
  note( -d $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254/no_cal/archive));
  ...
  cmp_ok( $recalibrated_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254/no_cal), 'recalibrated_path from summary link (no_cal)' );
  cmp_ok( $o->bustard_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254), 'bustard_path from summary link (no_cal)' );
...
{
  my $o = test::run::folder->new(
    archive_path => $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254/no_cal/archive),
  );
  cmp_ok( $o->recalibrated_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254/no_cal), 'recalibrated_path from archive_path' );
  cmp_ok( $o->bustard_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/PB_basecalls_20101016-172254), 'bustard_path from archive_path' );
...
{
  my $o = test::run::folder->new(
    archive_path => $hs_runfolder_dir . q(/Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2/GERALD_01-10-2010_RTA/archive),
  );
  cmp_ok($o->recalibrated_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2/GERALD_01-10-2010_RTA), 'recalibrated_path from archive_path' );
...
qx{rm $hs_runfolder_dir/Latest_Summary; ln -s Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2/GERALD_01-10-2010_RTA $hs_runfolder_dir/Latest_Summary};
{
  note( qx{ls -lh $hs_runfolder_dir} );
  my $linked_dir = readlink ( $hs_runfolder_dir . q{/Latest_Summary} );
  note $linked_dir;
  ...
  my $recalibrated_path;
  lives_ok { $recalibrated_path = $o->recalibrated_path; } 'recalibrated_path from runfolder_path and  summary link (gerald)';
  cmp_ok($recalibrated_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2/GERALD_01-10-2010_RTA), 'recalibrated_path from summary link (gerald)' );
  cmp_ok($o->bustard_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/Bustard1.8.1a2_01-10-2010_RTA.2), 'bustard_path from  summary link (gerald)' );
  cmp_ok($o->basecall_path, 'eq', $hs_runfolder_dir . q(/Data/Intensities/BaseCalls), 'basecall_path from  summary link (gerald)' );
  cmp_ok($o->runfolder_path, 'eq', $hs_runfolder_dir, 'runfolder_path from  summary link (gerald)' );
}

{
  my $tdir = catfile(tempdir( CLEANUP => 1 ), q());  #we need a trailing slash after our directory for the globbing later
