...

{
    ...
    is($r->start(), qq([[ -d $log_dir && -w $log_dir ]] && daemon -i -r -a 10 -n daemon --umask 002 -A 10 -L 10 -M 10 -o $log_dir/daemon-) . $hostname . qq[-$timestamp.log -- $command], 'start command on local host');
    is($r->ping(), q[daemon --running -n daemon && ((if [ -w /tmp/daemon.pid ]; then touch -mc /tmp/daemon.pid; fi) && echo -n 'ok') || echo -n 'not ok'], 'ping command on local host');
    is($r->stop(), q[daemon --stop -n daemon], 'stop command on local host');
}
...

{
    ...
    is($r->start(), qq([[ -d $log_dir && -w $log_dir ]] && daemon -i -r -a 10 -n daemon --env="PERL5LIB=/dodo/lib1:/dada/lib2" --umask 002 -A 10 -L 10 -M 10 -o $log_dir/daemon-$hostname-$timestamp.log -- $command), 'start command on local host');
