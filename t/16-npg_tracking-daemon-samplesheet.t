...

{
    ...
    cmp_deeply($r->libs, [qw(/software/solexa/lib/perl5)], 'libs defined');
    ...
    is($r->start(q[sf-1-1-01]), qq[daemon -i -r -a 10 -n npg_samplesheet_daemon --env="PERL5LIB=/software/solexa/lib/perl5" --umask 002 -A 10 -L 10 -M 10 -o $log_dir/npg_samplesheet_daemon-] . q[sf-1-1-01] . q[-2013.log -- perl -e 'use strict; use warnings; use npg::samplesheet::auto;  use Log::Log4perl qw(:easy); BEGIN{ Log::Log4perl->easy_init({level=>$INFO,}); } npg::samplesheet::auto->new()->loop();'], 'start command');
